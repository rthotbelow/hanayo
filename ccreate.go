...
func ccreateSubmit(c *gin.Context) {
	...
	if !ccreationEnabled() {
		ccreateResp(c, errorMessage{T(c, "โทษทีนะ, รู้สึกเหมือนจะสร้างแคลนตอนนี้ไม่ได้ ไว้ลองทีหลังนะ")})
		return
	}
	...
	if !cnameRegex.MatchString(name) {
		ccreateResp(c, errorMessage{T(c, "ชื่อแคลนสามารถมีตัวอักษร, ตัวเลข และอักษรพิเศษต่อไปนี้ <code>_[]-</code>.")})
		return
	}
	...
	if db.QueryRow("SELECT 1 FROM clans WHERE name = ?", c.PostForm("name")).
		...
		ccreateResp(c, errorMessage{T(c, "มีแคลนนั้นอยู่แล้วนะ!!")})
		return
	}
	...
	if db.QueryRow("SELECT 1 FROM clans WHERE tag = ?", c.PostForm("tag")).
		Scan(new(int)) != sql.ErrNoRows {
		ccreateResp(c, errorMessage{T(c, "มีตัวย่อแคลนนั้นอยู่แล้ว!!")})
		return
	}
	...
	if err != nil {
		ccreateResp(c, errorMessage{T(c, "ชิบหาย... อยู่ดีๆมีปัญหา แคลนอาจจะสร้างไปแล้วก็ได้ ไม่รู้ละว่าตอนนี้เกิดอะไรขึ้น")})
		fmt.Println(err)
		return
	}
